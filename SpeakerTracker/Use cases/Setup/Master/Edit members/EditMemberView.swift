...
class EditMemberView: UIView {
        ...
        @objc func saveButtonTapped(_: UIButton) {
            let member = Member(title: titleBox?.text, firstName: firstNameBox?.text, lastName: lastNameBox?.text, isGoverningBodyMember: nil)
            
            delegate?.saveButtonTapped(member: member)
        }
}
