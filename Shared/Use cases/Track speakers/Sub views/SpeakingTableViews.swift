         ...
         List {
            ForEach(viewModel, id: \.self){ sectionList in
               Section(header: HStack {
                  Text(sectionList.sectionHeader)
                     .font(.headline)
                     .foregroundColor(Color.black)
                     .padding(.vertical, 10.0)
                  if sectionList.sectionType == .amendment {
                     Spacer()
                     Text(">")
...
//                  if (sectionIsCollapsed[sectionList.sectionNumber] == false) || (sectionIsCollapsed.keys.contains(sectionList.sectionNumber) == false) {
